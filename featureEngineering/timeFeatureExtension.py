class TimeFeature():
    ...
    def extendTimeLagsFeatures(self, origin_df, columnNameList, n_lags, deleteFirstLagDuration= False):
        """
        This function generate time-lags-Features and extend original dataframe
        Example
        -------
        >>> from KETIPreDataTransformation.featureEngineering.timeFeatureExtension import TimeFeature
        >>> TF = TimeFeature()
        >>> lag = 100
        >>> columnNameList=['value']
        >>> df_generated = TF.extendTimeLagsFeatures(df, columnNameList, lag, deleteFirstLagDuration=True)
